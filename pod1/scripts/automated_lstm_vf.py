...
def run_lstm_model(input_file, output_base_path):
    ...
    df = df[['ETH']].sort_index()
    df['ETH'] = pd.to_numeric(df['ETH'], errors='coerce')
    df_returns = np.log(df / df.shift(1)).rename(columns={'ETH': 'ETH_returns'}).dropna()
    df_returns['realized_vol'] = df_returns['ETH_returns'].rolling(20).std().dropna()
